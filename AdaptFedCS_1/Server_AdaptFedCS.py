...
class Server_AdaptFedCS(object):
    ...
    def prepare_model_weights(self, weight, scalar=1):
        '''
        Multiply the local parameters of each client by its factor.
       ...
       # Initialize the first state
        ...
        if (init_weights != None) :
            ...
            for client in self.list_clients:
                ...
                # A: init Q_values
                lm = self.prepare_model_weights(client_w_for_first_iteration)
                self.model.load_state_dict(lm)
                acc4model, _ = self.test(type_data)
                Q_values[it] = acc4model.item()
                it += 1
        ...
        #print(len(weight_list_for_iteration))
        # print(Q_values)
        # get the weight with PCA
        weights_cosine_similarity = []
        global_weight_init = np.mean(weight_list_for_iteration, axis = 0)
        y = np.array(global_weight_init)
        y = y.reshape(1, -1)
        #print(global_weight_init)
        
        for i in range(0, len(self.list_clients)):
            x = np.array(weight_list_for_iteration[i])
            x = x.reshape(1, -1)
            
            #print('sim: ', cosine_similarity(x, y))
            weights_cosine_similarity.append(cosine_similarity(x, y)[0])

        cosine_sim = [x[0] for x in weights_cosine_similarity]
        #print(cosine_sim)
        ...
        for cpt in range(0, len(self.list_clients)):
            client_state = []
            client_state.append(cosine_sim[cpt])
        ...
        for comm_round in  range(comm_start, comms_round):
            #print(comm_round)
            #print(reputation_list[comm_round])

            rounds.append(comm_round+1)
            
            if (verbos == 1):
                print("*************************************************************************************")
                print("Communication round n others : ", comm_round + 1)
            ...
            # Q_values_t 
            # reputation_list[comm_round]

            # fit = []
            # for ac in active_clients:
            #     print('NoCores:', ac.numbercores)
            #     print('freq:', np.mean(ac.frequency))
            #     print('BW:', np.mean(ac.bandwidth))
            #     fit.append(ac.numbercores * np.mean(ac.frequency) * np.mean(ac.bandwidth))
            
            # print('fit', fit)
            # fit = minmax_scale(fit)
            # print('fit', fit)

            # A: fitness
            fit = []
            for ac_idx in active_clients_index:
                fit.append(Q_values[ac_idx])
            ...
            for idx,client_index in enumerate(active_clients_index):
                if (verbos == 1):
                    print("Entrainnement locale du client others: ", client.client_name)
                
                # A: estimate E
                it = E   
                if (fit[idx] >= th1):
                    it = 7
                elif(th1 >fit[idx]>th2):
                    it = 5
                else:
                    it = 3
                ...
                client_w = self.list_clients[client_index].train(global_weights, it, mu, type_data, verbos)
                all_client_w.append(client_w)

                # Append the local model weight
                weight_local_clients.append(self.flatten(client_w))

                # A: MAJ des weights PCA
                x = np.array(weight_local_clients[-1])
                
                x = x.reshape(1, -1)
                y = copy.deepcopy(global_weights)
                y = np.array(self.flatten(y))
                y = y.reshape(1, -1)
                
                #print(x, '\n',y)
                cosine_sim[client_index] = cosine_similarity(x,y)[0][0]
                #print(cosine_sim[client_index])
                state_list[client_index][0] =  cosine_sim[client_index] #list((pca.transform(np.array(self.flatten(copy.deepcopy(client_w))).reshape(1, -1)))[0])
                
                #print(state_list[client_index][0])
            ...
            # A: accuracy for all clients.
            acc_all_models = []
            for lm in all_client_w:
                lm = self.prepare_model_weights(lm)
                self.model.load_state_dict(lm)
                acc4model, _ = self.test(type_data)
                acc_all_models.append(acc4model.item())
            ...
            alpha = 0.7
            beta = 0.3
            for idx, ac_idx in enumerate(active_clients_index):
                delay = (time_roundt[idx] -min(time_roundt)) / (max(time_roundt) - min(time_roundt))
                acc = acc_all_models[idx]  
                #print('delay & acc: ', delay, acc)  

                Q_t = Q_values[ac_idx]  
                # Q_values[ac_idx] = 0.5 * Q_t + alpha * acc - beta * delay
                Q_values[ac_idx] = alpha * acc - beta * delay
            ...
            if (verbose_test == 1):
                print("Training round n others:", (comm_round+1),", Test accuarcy : ", round(Accuray_global_t.numpy()*100, 2) ,", Test loss :", round(loss_test, 2))
                print("*************************************************************************************")
    ...
    def global_train_fall(self, comms_round, C, E, mu, lamb, rep_init, batch_size, verbose_test = 1, verbos = 0, type_data = "others",
       ...
       # Initialize the first state
        ...
        for comm_round in  tqdm(range(comm_start, comms_round)):
            ...
            if (verbos == 1):
                print("*************************************************************************************")
                print("Communication round n fall: ", comm_round + 1)
            ...
            for client_index in active_clients_index:
                if (verbos == 1):
                     print("Entrainnement locale du client fall: ", client.client_name)
            ...
            acc_all_models = []
            for all_models in weight_local_clients:
                self.model.load_state_dict(all_models)
                acc4model = self.test(type_data)
                acc_all_models.append(acc4model)
            ...
            if (verbose_test == 1):
                print("Training round n fall :", (comm_round+1),", Test accuarcy : ", round(acc_test*100, 2) ,", Test F1_score :", round(F1score_global_t*100, 2)," Test loss :", round(loss_test, 2))
